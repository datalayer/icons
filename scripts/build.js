...
/**
 * Write the barrels of the package root, over every flavor.
 *
 * They used to be written by hand, and said `export * from "./data1"` twice —
 * so `data2` and `eggs` were unreachable from the root, and no build ever
 * corrected it. Worse, a star export is silently AMBIGUOUS: a name exported
 * by two flavors — `AppleRainbowIcon`, `StudentIcon`, `UserIcon`, `UsersIcon`
 * and `AngularIcon` are in both `data1` and `data2` — is dropped from the
 * namespace altogether rather than resolved, so those icons existed in the
 * package and reached nobody.
 *
 * The barrels are therefore generated, exhaustively and by name: every icon
 * of every flavor is re-exported explicitly, which leaves nothing ambiguous.
 * A name carried by more than one flavor is taken from the first that has it,
 * `flavors` being in order of precedence; the other one keeps its own path —
 * `@datalayer/icons-react/data2/AppleRainbowIcon` — and the gallery reads the
 * flavors one by one, so both drawings stay visible.
 *
 * @param {string} package Package being built
 * @param {string[]} flavors Flavors, in order of precedence
 */
async function buildRootIndexes(package, flavors) {
  /** @type {{ componentName: string, flavor: string }[]} */
  const exported = [];
  const claimed = new Map();
  const shadowed = [];
  for (const flavor of flavors) {
    for (const { componentName } of await getIcons(flavor)) {
      const owner = claimed.get(componentName);
      if (owner) {
        shadowed.push(`${flavor}/${componentName} (kept ${owner})`);
        continue;
      }
      claimed.set(componentName, flavor);
      exported.push({ componentName, flavor });
    }
  }
  exported.sort((a, b) => a.componentName.localeCompare(b.componentName));

  const banner =
    "// Generated by scripts/build.js — do not edit.\n" +
    `// ${exported.length} icons from ${flavors.join(", ")}.\n`;
  const esm = exported
    .map(
      ({ componentName, flavor }) =>
        `export { default as ${componentName} } from './${flavor}/esm/${componentName}.js';`,
    )
    .join("\n");
  const cjs = exported
    .map(
      ({ componentName, flavor }) =>
        `module.exports.${componentName} = require("./${flavor}/${componentName}.js");`,
    )
    .join("\n");
  const types = exported
    .map(
      ({ componentName, flavor }) =>
        `export { default as ${componentName} } from './${flavor}/${componentName}';`,
    )
    .join("\n");

  await Promise.all([
    ensureWrite(`./${package}/index.esm.js`, `${banner}${esm}\n`),
    ensureWrite(`./${package}/index.js`, `${banner}${cjs}\n`),
    ensureWrite(`./${package}/index.d.ts`, `${banner}${types}\n`),
  ]);

  console.log(
    `Root barrels: ${exported.length} icons from ${flavors.join(", ")}.`,
  );
  /*
   * A name in two flavors is worth saying out loud.
   *
   * Only one of the two can answer to the bare name at the root, so the other
   * is reachable by its own path alone — and whoever draws the icons is the
   * one who can rename it. Said as a warning rather than a line of the log,
   * because it is a decision taken on their behalf.
   */
  if (shadowed.length) {
    console.warn(
      `\n⚠️  WARNING — ${shadowed.length} icon name${
        shadowed.length > 1 ? "s are" : " is"
      } carried by more than one flavor.\n` +
        `    At the root of the package, the bare name is the one of the first\n` +
        `    flavor in ${flavors.join(" > ")}; the other is reachable only by its\n` +
        `    own path, e.g. "@datalayer/icons-react/data2/AppleRainbowIcon".\n` +
        `    Rename one of the two SVG files to give both a name of their own:\n` +
...
async function main(package) {
  ...
  await buildRootIndexes(package, ["data1", "data2", "eggs"]);
